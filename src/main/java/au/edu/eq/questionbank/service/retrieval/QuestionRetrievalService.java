...
public final class QuestionRetrievalService {
	...
	private List<QuestionRetrievalResult> retrieveForCurrentNodes(List<CurriculumNode> currentNodes) {
		// Persistent-ID ordering keeps results stable regardless of repository match order.
		...
		for (QuestionApplicabilityMatch match : matches) {
			...
			// Repository results must not broaden the curriculum scope selected by the caller.
			...
			// Merge repeated matches into one question while retaining each distinct reason it matched.
