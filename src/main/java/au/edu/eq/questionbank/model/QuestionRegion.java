 ...
 * <p>
 * Page numbers are human-readable and one-based. The coordinates use a
 * top-left origin and are normalized fractions of the rendered page: {@code x}
 * and {@code y} locate the top-left corner, while {@code width} and
 * {@code height} describe the region's extent. For example, {@code x = 0.10}
 * means ten percent of the rendered page width from its left edge.
 * <p>
 * Coordinates refer to the page as displayed after the renderer has applied
 * the PDF page's crop box and rotation. They do not represent PDF user-space
 ...
 * @param x the normalized horizontal position of the region's left edge
 ...
 * coordinate is not finite, or the region has no positive area within the
 * normalized page bounds
 */
public record QuestionRegion(int pageNumber, double x, double y, double width, double height) {
