...
### Mixed-content legacy split capture

Legacy Question split capture remains intentionally PDF-region-only.
Clipboard images are blocked while the split workflow is active, so the
current implementation cannot silently discard a pasted image.

Consider extending `LegacyQuestionSplitService.SplitPart` and its atomic
split transaction to ordered `QuestionContentPart` values only if real
legacy-corpus use demonstrates a need to split Questions containing
stored images. Any such change must preserve mixed image/PDF order and
retain the existing all-or-nothing split semantics.
